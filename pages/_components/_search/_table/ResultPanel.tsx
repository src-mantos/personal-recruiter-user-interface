...
export interface ResultPanelProps extends StylableComponent, AnyMap {
    layoutWidth: number;
}
...
const ResultPanel = (props:ResultPanelProps) => {
    ...
    const data = (postResultSet == undefined)? []:postResultSet;

    const tableInst = useTable(
        { columns, data, filterTypes }, 
        useFlexLayout);//useFlexLayout, useFilters, useSortBy, useRowSelect

    const { getTableProps, rows, prepareRow } = tableInst;

    console.log("instance",tableInst);
    const {style:g2, ...TableProps} = getTableProps();

    return (
        <div key={"result-panel"} {...TableProps} className={["is-flex is-flex-direction-column", props.className, styles["rs-table"]].join(" ")}>
            
            <TableHeader instance={tableInst}></TableHeader>

            {rows.map((row) => {
                prepareRow(row);
                return ( <TableRow instance={tableInst} row={row}></TableRow>);
            })}
        </div>
    );
    ...
    return (            
        <div key={"RS-TableHeader"} className={["is-flex",styles["rs-row"],styles["rs-header"]].join(" ")}>
            ...
            {headers.map( (column) => {
                ...
                return (
                    <div key={"headerRow_"+column.id}  className={["is-flex is-flex-direction-column",styles["rs-row"], styles["rs-cell"],colClassName].join(" ")}>
